...
async function issueZPass(privateKey, inputs) {
  ...
  const merkeRoot = await zpass.getMerkleRoot(inputs);
  const leafHashes = await zpass.getLeavesHashes(inputs);
  const leavesHashesString = `[${leafHashes.join(",")}]`;

  const sig = await zpass.signMerkleRoot(merkeRoot);
